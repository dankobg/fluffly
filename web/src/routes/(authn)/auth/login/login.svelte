<script lang="ts">
	...
	const supForm = superForm(initialLoginForm, {
		...
		async onUpdated({ form }) {
			if (!form.valid) {
				sessionStorage.removeItem('fluffly_email_verified');
			...
			if (url) {
				try {
					const successfulLogin = await kratos.updateLoginFlow({
						flow: data.flow?.id ?? '',
						updateLoginFlowBody: body
					});
					if (successfulLogin.continue_with) {
						for (const item of successfulLogin.continue_with) {
							switch (item.action) {
	...
	$effect(() => {
		const val = window.sessionStorage.getItem('fluffly_email_verified');
		if (val) {
			emailVerified = true;
		...
		<Card.Content>
			<div class="grid gap-4">
				<form method="POST" use:enhance class="grid gap-4">
					...
					{#if emailVerified}
						<Alert.Root variant="success" icon>
