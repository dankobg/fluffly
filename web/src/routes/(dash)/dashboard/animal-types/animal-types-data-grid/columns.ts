...
export const columns: ColumnDef<components['schemas']['AnimalType']>[] = [
	{
		id: 'select',
		header: ({ table }) =>
	...
	},
	{
		accessorKey: 'id',
		header: ({ column }) => {
			return renderComponent(DataTableColumnHeader<components['schemas']['AnimalType'], unknown>, {
				column,
				title: 'Id'
			});
		},
		cell: ({ row }) => {
			return renderComponent(DataTableCellId, {
				...
				href: `/dashboard/animal-types/${row.original.id}`
			});
		},
		enableSorting: false,
		enableHiding: false
	...
	{
		id: 'actions',
		cell: ({ row }) => renderComponent(DataTableRowActions<components['schemas']['AnimalType']>, { row })
	}
];
