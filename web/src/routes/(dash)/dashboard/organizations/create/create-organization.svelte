<script lang="ts">
	...
	import { OrganizationStatus, type components } from '$lib/gen/fluffly_openapi';
	...
	onMount(() => {
		...
		uppyPhotos.use(UppyDashboard, {
			inline: true,
			theme: 'auto',
			target: '#create-organization-photos',
		...
		uppyVideos.use(UppyDashboard, {
			inline: true,
			theme: 'auto',
			target: '#create-organization-videos',
	...
	let createdOrganization: components['schemas']['Organization'] | null = $state(null);
	...
	export const createOrganizationFormSchema = v.pipe(
		v.object({
			...
			status: v.picklist(organizationStatusValues, 'status is required'),
			work_hour: v.pipe(
				v.optional(
					v.object({
						monday: v.optional(
	...
	type CreateOrganizationFormInput = v.InferInput<typeof createOrganizationFormSchema>;
	type CreateOrganizationFormOutput = v.InferOutput<typeof createOrganizationFormSchema>;

	const initialCreateOrganizationForm: CreateOrganizationFormInput = {
		name: '',
	...
	const supForm = superForm(initialCreateOrganizationForm, {
		id: 'create_organization',
		validators: valibot(createOrganizationFormSchema),
		SPA: true,
		dataType: 'json',
		...
		async onUpdate({ form }) {
			...
			try {
				const organizationResult = await fluffly.POST('/organizations', {
					body: {
						data: form.data
					},
					bodySerializer(body) {
						const fd = new FormData();
						const data = body?.data as CreateOrganizationFormOutput | undefined;
						if (!data) {
				...
				if (organizationResult.data) {
					createdOrganization = organizationResult.data;
				...
				if (organizationResult.error) {
					...
					if (organizationResult.error.code === 'validation') {
						...
						const fieldErrors: ValidationErrors<CreateOrganizationFormInput> = {};
...
<div class="flex h-full flex-1 flex-col space-y-8 p-8">
	<div class="flex items-center justify-between space-y-2">
		<div>
			<h1 class="text-2xl font-bold tracking-tight">Create organization</h1>
			<!-- <p class="text-muted-foreground">Create new organization</p> -->
		</div>
	</div>
	<div>
		<form method="POST" use:enhance enctype="multipart/form-data" class="grid gap-4">
			...
			<div class="col-span-full">
				...
				<div id="create-organization-photos"></div>
			...
			<div class="col-span-full">
				...
				<div id="create-organization-videos"></div>
			...
			<div class="mt-4 ml-auto">
				<Form.Button class="font-bold">create organization</Form.Button>
			</div>
		</form>
	</div>
	...
	{#if createdOrganization}
		<Card.Root class="max-w-6xl">
			<Card.Header>
				<Card.Title>View created organization</Card.Title>
			</Card.Header>
			...
			<Card.Content>
				<div class="grid grid-cols-1 gap-x-8 gap-y-4 text-sm sm:grid-cols-2">
					<div class="flex flex-col justify-center">
						...
						<a class="font-medium text-sky-500 underline" href="/dashboard/organizations/{createdOrganization.id}">
							{createdOrganization.id}
						</a>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Name</span>
						<span class="font-medium">{createdOrganization.name}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Website</span>
						<span class="font-medium">{createdOrganization.website}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Mission statement</span>
						<span class="font-medium">{createdOrganization.mission_statement}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Adoption policy</span>
						<span class="font-medium">{createdOrganization.adoption_policy}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Adoption URL</span>
						<span class="font-medium">{createdOrganization.adoption_url}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Created time</span>
						<time class="font-medium">{fmt.format(new Date(createdOrganization.created_at))}</time>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Updated time</span>
						<time class="font-medium">{fmt.format(new Date(createdOrganization.updated_at))}</time>
					</div>
				</div>
			</Card.Content>
		</Card.Root>
