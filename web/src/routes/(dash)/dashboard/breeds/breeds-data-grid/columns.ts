...
export const columns: ColumnDef<components['schemas']['Breed']>[] = [
	{
		id: 'select',
		header: ({ table }) =>
	...
	},
	{
		accessorKey: 'id',
		header: ({ column }) => {
			return renderComponent(DataTableColumnHeader<components['schemas']['Breed'], unknown>, {
				column,
				title: 'Id'
			});
		},
		cell: ({ row }) => {
			return renderComponent(DataTableCellId, {
				...
				href: `/dashboard/breeds/${row.original.id}`
			});
		},
		enableSorting: false,
		enableHiding: false
	...
	{
		accessorKey: 'animal_specie_id',
		header: ({ column }) => {
			return renderComponent(DataTableColumnHeader<components['schemas']['Breed'], unknown>, {
				title: 'Animal species ID',
				column
			});
		},
		cell: ({ row }) => {
			const animalSpecieIdSnippet = createRawSnippet<[{ animal_specie_id: number }]>(getAnimalSpecieId => {
				const { animal_specie_id } = getAnimalSpecieId();
				return {
					render: () => `<div>${animal_specie_id}</div>`
				};
			});
			return renderSnippet(animalSpecieIdSnippet, {
				animal_specie_id: row.original.animal_specie_id
			});
		},
		enableSorting: false
	},
	...
	{
		id: 'actions',
		cell: ({ row }) => renderComponent(DataTableRowActions<components['schemas']['Breed']>, { row })
	}
];
