...
import {
	...
	AnimalGender,
	AnimalSize,
	PathsMeAnimalsAdoptedGetParametersQueryEmbed,
	type operations
...
export const load: PageLoad = async ({ fetch, url, depends }) => {
	depends('data:dashboard-adopted-animals');

	const listMyAdoptionsParams: operations['listMyAdoptions']['parameters'] = {
		query: {
			embed: [PathsMeAnimalsAdoptedGetParametersQueryEmbed.breeds, PathsMeAnimalsAdoptedGetParametersQueryEmbed.tags]
		}
	};
	...
	if (pageParam) {
		...
		if (!Number.isNaN(page)) {
			listMyAdoptionsParams.query!.page = page;
		}
	}
	...
	if (pageSizeParam) {
		...
		if (!Number.isNaN(pageSize)) {
			listMyAdoptionsParams.query!.page_size = pageSize;
		}
	}
	...
	if (animalTypeIdParam) {
		...
		if (!Number.isNaN(animalTypeId)) {
			listMyAdoptionsParams.query!.animal_type_id = [animalTypeId];
		}
	}
	...
	if (animalSpecieIdParam) {
		...
		if (!Number.isNaN(animalSpecieId)) {
			listMyAdoptionsParams.query!.animal_specie_id = [animalSpecieId];
		}
	}
	...
	if (animalBreedsIdParams.length > 0) {
		...
		if (animalBreedIds.length > 0) {
			listMyAdoptionsParams.query!.animal_breed_id = animalBreedIds;
		}
	}
	...
	if (ageParams?.length > 0) {
		...
		if (ages.length > 0) {
			listMyAdoptionsParams.query!.age = ages;
		}
	}
	...
	if (sizeParams?.length > 0) {
		...
		if (sizes.length > 0) {
			listMyAdoptionsParams.query!.size = sizes;
		}
	}
	...
	if (daysGtParam) {
		...
		if (!Number.isNaN(daysGt)) {
			listMyAdoptionsParams.query!.days_gt = daysGt;
		}
	}
	const sortParams = url.searchParams.getAll('sort');
	if (sortParams.length > 0) {
		listMyAdoptionsParams.query!.sort = sortParams;
	}
	...
	if (Object.keys(properties).length > 0) {
		listMyAdoptionsParams.query!.properties = properties;
	}

	try {
		const animalsResult = await fluffly.GET('/me/animals/adopted', {
			fetch,
