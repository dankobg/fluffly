...
import {
	...
	AnimalGender,
	AnimalSize,
	PathsMeAnimalsFavoriteGetParametersQueryEmbed,
	type operations
...
export const load: PageLoad = async ({ fetch, url, depends }) => {
	depends('data:dashboard-favorite-animals');

	const listMyFavoriteAnimalsParams: operations['listMyFavoriteAnimals']['parameters'] = {
		query: {
			embed: [PathsMeAnimalsFavoriteGetParametersQueryEmbed.breeds, PathsMeAnimalsFavoriteGetParametersQueryEmbed.tags],
			sort: ['-liked_at']
		}
	};
	...
	if (pageParam) {
		...
		if (!Number.isNaN(page)) {
			listMyFavoriteAnimalsParams.query!.page = page;
		}
	}
	...
	if (pageSizeParam) {
		...
		if (!Number.isNaN(pageSize)) {
			listMyFavoriteAnimalsParams.query!.page_size = pageSize;
		}
	}
	...
	if (animalTypeIdParam) {
		...
		if (!Number.isNaN(animalTypeId)) {
			listMyFavoriteAnimalsParams.query!.animal_type_id = [animalTypeId];
		}
	}
	...
	if (animalSpecieIdParam) {
		...
		if (!Number.isNaN(animalSpecieId)) {
			listMyFavoriteAnimalsParams.query!.animal_specie_id = [animalSpecieId];
		}
	}
	...
	if (animalBreedsIdParams.length > 0) {
		...
		if (animalBreedIds.length > 0) {
			listMyFavoriteAnimalsParams.query!.animal_breed_id = animalBreedIds;
		}
	}
	...
	if (ageParams?.length > 0) {
		...
		if (ages.length > 0) {
			listMyFavoriteAnimalsParams.query!.age = ages;
		}
	}
	...
	if (sizeParams?.length > 0) {
		...
		if (sizes.length > 0) {
			listMyFavoriteAnimalsParams.query!.size = sizes;
		}
	}
	...
	if (daysGtParam) {
		...
		if (!Number.isNaN(daysGt)) {
			listMyFavoriteAnimalsParams.query!.days_gt = daysGt;
		}
	}
	const sortParams = url.searchParams.getAll('sort');
	if (sortParams.length > 0) {
		listMyFavoriteAnimalsParams.query!.sort = sortParams;
	}
	...
	if (Object.keys(properties).length > 0) {
		listMyFavoriteAnimalsParams.query!.properties = properties;
	}

	try {
		const animalsResult = await fluffly.GET('/me/animals/favorite', {
			fetch,
