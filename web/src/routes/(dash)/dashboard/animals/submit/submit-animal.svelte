<script lang="ts">
	...
	let submitedAnimal: components['schemas']['Animal'] | null = $state(null);
	let submitedAnimalTypeName = $state<string>('');
	let submitedAnimalSpeciesName = $state<string>('');
	let submitedAnimalBreedNames = $state<string>('');
	...
	let selectedOrganizationName = $derived(
		data?.myOrganizationsResult?.data?.data.find(x => x.id === $form.organization_id)?.name
	);
	...
	export const submitAnimalFormSchema = v.pipe(
		v.object({
	...
	type SubmitAnimalFormInput = v.InferInput<typeof submitAnimalFormSchema>;
	type SubmitAnimalFormOutput = v.InferOutput<typeof submitAnimalFormSchema>;

	const initialSubmitAnimalForm: SubmitAnimalFormInput = {
	...
	const supForm = superForm(initialSubmitAnimalForm, {
		id: 'submit_animal',
		validators: valibot(submitAnimalFormSchema),
		SPA: true,
		dataType: 'json',
		...
		async onUpdate({ form }) {
			...
			try {
				const animalResult = await fluffly.POST('/animals/submissions', {
					body: {
						data: form.data as SubmitAnimalFormOutput
					},
					bodySerializer(body) {
						const fd = new FormData();
						const data = body?.data as SubmitAnimalFormOutput | undefined;
						if (!data) {
				...
				if (animalResult.data) {
					submitedAnimal = animalResult.data;
					submitedAnimalTypeName = selectedAnimalTypeName || '';
					submitedAnimalSpeciesName = selectedAnimalSpecie?.name || '';
					submitedAnimalBreedNames = selectedBreedNames || '';

					toast.success('Animal submited');
					reset();
					uppyImage?.clear();
					uppyPhotos?.clear();
					uppyVideos?.clear();
				...
				if (animalResult.error) {
					...
					if (animalResult.error.code === 'validation') {
						...
						const fieldErrors: ValidationErrors<SubmitAnimalFormInput> = {};
...
<div class="flex h-full flex-1 flex-col space-y-8 p-8">
	<div class="flex items-center justify-between space-y-2">
		<div>
			<h1 class="text-2xl font-bold tracking-tight">Submit animal</h1>
			<!-- <p class="text-muted-foreground">Submit new animal</p> -->
		</div>
	</div>
	<div>
		<form method="POST" use:enhance enctype="multipart/form-data" class="grid gap-4">
			<section class="grid gap-4 md:grid-cols-2">
				...
				{#if data.myOrganizationsResult?.data?.data && data.myOrganizationsResult.data.data.length > 0}
					<div class="grid gap-2">
						<div class="flex items-center gap-2">
							<Checkbox
								id="submit-animal-owner-is-org"
								checked={ownerIsOrganization}
								onCheckedChange={v => {
									ownerIsOrganization = v;
									if (!v) {
							...
							<div class="space-y-1 leading-none">
								<Label for="submit-animal-owner-is-org">Posted by organization</Label>
							</div>
						</div>
					</div>
				{/if}
				...
				<div class="grid gap-2">
					<div class="flex items-center gap-2">
						<Checkbox bind:checked={hasMicrochip} id="submit-has-microchip" />
						<div class="space-y-1 leading-none">
							<Label for="submit-has-microchip">Has microchip</Label>
						</div>
					</div>
				</div>
				...
				<div class="col-span-full mt-8">
					...
					<div id="submit-animal-image"></div>
				...
				<div class="col-span-full mt-8">
					...
					<div id="submit-animal-photos"></div>
				...
				<div class="col-span-full mt-8">
					...
					<div id="submit-animal-videos"></div>
				...
				<div class="mt-4 ml-auto">
					<Form.Button class="font-bold">Submit animal</Form.Button>
				</div>
			</section>
		</form>
	</div>
	...
	{#if submitedAnimal}
		<Card.Root class="max-w-6xl">
			<Card.Header>
				<Card.Title>View submited animal</Card.Title>
			</Card.Header>
			...
			<Card.Content>
				<div class="grid grid-cols-1 gap-x-8 gap-y-4 text-sm sm:grid-cols-2">
					<div class="flex flex-col justify-center">
						...
						<a class="font-medium text-sky-500 underline" href="/dashboard/animals/{submitedAnimal.id}">
							{submitedAnimal.id}
						</a>
					</div>
					...
					<div class="flex flex-col justify-center">
						...
						<span class="font-medium">{submitedAnimal.name}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Type</span>
						<span class="font-medium">{submitedAnimalTypeName}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Species</span>
						<span class="font-medium">{submitedAnimalSpeciesName}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Breeds</span>
						<span class="font-medium">{submitedAnimalBreedNames}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Gender</span>
						<span class="font-medium">{submitedAnimal.gender}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Hermaphrodite</span>
						<span class="font-medium">{submitedAnimal.hermaphrodite}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Age</span>
						<span class="font-medium">{submitedAnimal.age}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Size</span>
						<span class="font-medium">{submitedAnimal.size}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Status</span>
						<span class="font-medium">{submitedAnimal.status}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Description</span>
						<span class="font-medium">{submitedAnimal.description}</span>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Created time</span>
						<time class="font-medium">{fmt.format(new Date(submitedAnimal.created_at))}</time>
					</div>
					<div class="flex flex-col justify-center">
						<span class="text-muted-foreground">Updated time</span>
						<time class="font-medium">{fmt.format(new Date(submitedAnimal.updated_at))}</time>
					</div>
				</div>
			</Card.Content>
		</Card.Root>
