...
<header class="sticky top-0 z-50 mx-auto flex w-full max-w-[120rem] items-center border-b bg-background">
	<div class="flex h-(--header-height) w-full items-center gap-2 px-4">
		<Button class="size-8 sm:hidden" variant="ghost" size="icon" onclick={sidebar.toggle}>
			<IconSidebar />
		</Button>
		<Separator orientation="vertical" class="me-2 h-4 sm:hidden" />
		...
		<div class="hidden items-center gap-2 sm:flex">
			{#each mainNavItems as item (item.title)}
				<a
					...
					class:text-primary={page.url.pathname === item.url}>{item.title}</a
				>
			{/each}
		</div>
